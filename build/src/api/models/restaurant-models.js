...
exports.getAllRestaurantTypes = exports.updateRestaurant = exports.createRestaurant = exports.getRestaurantSummary = exports.getRestaurantsByRating = exports.getRestaurants = exports.getCurrentRestaurantInfoByName = void 0;
const tslib_1 = require("tslib");
const client_1 = tslib_1.__importDefault(require("../../config/client"));
const utils_1 = require("../../utils");
const s3_1 = require("../../services/aws/s3");
const { restaurant } = client_1.default;
...
const getRestaurants = async (query_params, limit, orderBy) => {
...
const getRestaurantsByRating = async (query_params, limit) => {
    const { city, country } = query_params;
    const restaurants = await client_1.default.$queryRaw `
        SELECT
			r.name,
    ...
    const restaurantPromises = restaurants.map(async (restaurant) => {
        const headersUrlPromises = restaurant
            ? restaurant.headers_path.map((path) => {
...
exports.getRestaurantsByRating = getRestaurantsByRating;
const getRestaurantSummary = async (restaurant_id) => {
    const query = await client_1.default.restaurant.findUnique({
        where: {
            id: restaurant_id,
        },
        select: {
    ...
    if (query) {
        const price_average = (0, utils_1.calculatePriceAverage)(query.dishes);
        const rating_average = (0, utils_1.calculateRatingAverage)(query.ratings);
        return {
            price_average,
...
exports.getRestaurantSummary = getRestaurantSummary;
const createRestaurant = async (payload) => {
    const query = await client_1.default.$transaction(async (tx) => {
...
const updateRestaurant = async (restaurant_id, payload) => {
    const query = await client_1.default.$transaction(async (tx) => {
...
const getAllRestaurantTypes = async () => {
    const query = await client_1.default.restaurantInformation
        .findMany({
...
exports.getAllRestaurantTypes = getAllRestaurantTypes;
