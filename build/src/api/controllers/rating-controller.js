...
let RatingController = class RatingController extends tsoa_1.Controller {
    async getMyRating(authorization, status, city, search) {
        const query_params = {
            status,
            city,
            search,
        };
        await (0, rating_validations_1.getMyRatingsValidations)(authorization);
        return (0, rating_services_1.getMyRatingsService)(authorization, query_params);
    }
    async getRatings(restaurant_name) {
        await (0, rating_validations_1.getRestaurantRatingsValidations)(restaurant_name);
        return (0, rating_services_1.getRestaurantRatingsService)(restaurant_name);
    }
    async getRatingStats(restaurant_name) {
        await (0, rating_validations_1.getRestaurantRatingStatsValidations)(restaurant_name);
        return (0, rating_services_1.getRestaurantRatingStatsService)(restaurant_name);
    }
    async putRating(authorization, rating_id, rating_record) {
        await (0, rating_validations_1.putRatingValidations)(authorization, rating_id);
